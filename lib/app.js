...
var hsParser = function hsParser(cardname, callback) {
	...
	_unirest2.default.get('https://omgvamp-hearthstone-v1.p.mashape.com/cards/search/' + cardname + '?locale=itIT').header("X-Mashape-Key", "QNONu0GxUCmshtEPsWRc3xvZ2EYup11KhpejsnUWCeuk7rphhd").end(function (result) {
	...
	if (e.message.content.split(' ')[0] == '!dio') bestParser(function (text) {
		...
		hsParser(parameter, function (found, response) {
			...
			if (found) {
				var message = "";
				var img = void 0;
				if (response.length > 1) message = e.message.author.mention + ': Sono stati trovati `' + response.length + '` per la chiave di ricerca `' + parameter + '`.\n';else message = e.message.author.mention + ': È stato trovato `' + response.length + '` per la chiave di ricerca `' + parameter + '`.\n';
				var _i = 0;
				...
				if (img != undefined) {
					(function () {
						...
						download(img, filename, function () {
							...
							_fs2.default.unlink('./img_temp/' + filename);
						});
					})();
				} else {
